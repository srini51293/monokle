...
Above we can see the 10 resources included by the selected kustomization.
...
Above we see two "upstream" kustomizations that include the selected kustomization in their configuration.
...
In the screenshot below we can see that the selected kustomization resulted in 7 resources that contain references to the image
